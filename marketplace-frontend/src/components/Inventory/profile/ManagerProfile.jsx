...
const ManagerProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true)
      const response = await inventoryService.getProfile()
      ...
      setFormData({
        ...
        assignedWarehouse: response.data.assignedWarehouse,
      })
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Manager Profile</h2>
          ...
          onClick={() => {
            if (editing) {
              ...
              setFormData({
                ...
                assignedWarehouse: profile.assignedWarehouse,
              })
            } else {
              setEditing(true)
            }
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-gray-600">Inventory Manager</p>
            <p className="mt-2 text-sm text-gray-500">Employee: {profile.employeeCode}</p>
            <p className="text-xs text-gray-400">ID: {profile.inventoryManagerId}</p>
          </div>
          <div className="pt-6 mt-6 border-t border-gray-200">
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Account Status</span>
                <span className={`font-semibold ${profile.isActive ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">Assigned Warehouse</p>
                    <p className="text-sm text-gray-600">{profile.assignedWarehouse || "Not specified"}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{profile.employeeCode}</p>
                  </div>
                </div>
              </div>
            </div>
